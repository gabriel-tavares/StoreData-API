...
class AppleAppStoreScraper {
  ...
  async getReviewsWithReplies(appId, country = 'br') {
    ...
    try {
      ...
      // Aguardar carregamento das reviews
      console.log('⏳ Aguardando carregamento das reviews...');
      await page.waitForSelector('.we-customer-review', { timeout: 10000 }).catch(() => {
        console.log('⚠️ Seletor padrão não encontrado, continuando...');
      });
      ...
      for (let i = 0; i < 10; i++) {
        ...
        // Verificar se há mais reviews carregadas
        const currentReviews = await page.evaluate(() => {
          return document.querySelectorAll('.we-customer-review').length;
        });
        console.log(`📊 Reviews encontradas após scroll ${i + 1}: ${currentReviews}`);
      }
      
      // Tentar clicar em diferentes botões de "Ver mais reviews"
      const seeMoreSelectors = [
        'button[aria-label*="Ver mais"]',
        'button[aria-label*="See more"]',
        '.we-customer-reviews__see-all',
        'button:contains("Ver mais")',
        'button:contains("See more")',
        '.we-customer-reviews button',
      ...
      for (const selector of seeMoreSelectors) {
        try {
          ...
          if (button) {
            console.log(`🔄 Clicando em botão "Ver mais" (${selector})...`);
            await button.click();
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            // Verificar se mais reviews foram carregadas
            const newCount = await page.evaluate(() => {
              return document.querySelectorAll('.we-customer-review').length;
            });
            console.log(`📊 Reviews após clique: ${newCount}`);
          }
        } catch (error) {
          console.log(`ℹ️ Erro ao tentar clicar em ${selector}:`, error.message);
        }
      }
      
      // Extrair TODAS as avaliações (com e sem respostas)
      const reviewsWithReplies = await page.evaluate(() => {
        ...
        console.log(`🔍 Total de elementos de review encontrados: ${reviewElements.length}`);
        
        // Se não encontrar com o seletor padrão, tentar outros seletores
        let allReviewElements = reviewElements;
        if (reviewElements.length < 10) {
          console.log('🔄 Tentando seletores alternativos...');
          const altSelectors = [
            '.we-customer-reviews .we-customer-review',
            '.l-row .we-customer-review',
            '[data-testid*="review"]',
            '.customer-review'
          ];
          ...
          for (const selector of altSelectors) {
            ...
            console.log(`📊 Seletor "${selector}": ${altElements.length} elementos`);
            if (altElements.length > allReviewElements.length) {
              allReviewElements = altElements;
            }
          }
        }
        
        allReviewElements.forEach((reviewEl, index) => {
          try {
            ...
            // Converter data de DD/MM/YYYY para MM/DD/YYYY para parsing correto
